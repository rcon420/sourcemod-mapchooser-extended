...
public OnPluginStart()
{
	...
	CreateTimer(10.0, Timer_RecheckCFG, _, TIMER_REPEAT);
...
public Action Timer_RecheckCFG(Handle timer){
	OnConfigsExecuted();
}
...
public OnConfigsExecuted()
{
	...
	for(int i = 0; i < GetArraySize(g_MapList); i++){
		char str[128];
		GetArrayString(g_MapList, i, str, sizeof str);
		PrintToServer("%s", str[i]);
	}
	*/
